  ...
  if (reviewMode) {
    return (
      <div className="min-h-screen bg-slate-950 text-white">
        <div className="sticky top-0 z-10 bg-slate-950/90 backdrop-blur border-b border-slate-800 px-6 py-4 flex items-center justify-between">
          <div>
          ...
          <div className="flex flex-row items-center space-x-2 rounded-full border border-white px-4 py-2 cursor-pointer">
            <Printer size={16} />{" "}
        ...
        >
          ...
          <div className="flex flex-row items-center justify-between">
            ...
            <p>সময়ঃ {paper.duration}</p>
          </div>
          {paper.questions.map((q, index) => {
            const userAnswer = answers[q._id];
            const isCorrect = userAnswer === q.correctOption;
            return (
              <div key={q._id} className="bg-white overflow-hidden">
                <div className="flex items-start gap-4 ">
                  <span className="flex-shrink-0 w-8 h-8 rounded-lg  flex items-center justify-center  font-mono text-sm font-bold text-black">
                    {index + 1}
                  </span>
